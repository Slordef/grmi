...
describe('RunnerManager', () => {
    it('should build docker runner', async () => {
        ...
        new RunnerManager(
            new TestFetcher(mockFetch)
        );
        await new Promise<void>(r => {
            called.spawn = r;
        });
        expect(mockFetch).toBeCalledTimes(1);
        expect(mockFetch).toBeCalledWith('https://api.github.com/repos/actions/runner/releases/latest', undefined);
        expect(mockSpawn).toBeCalledTimes(1);
        expect(mockSpawn).toBeCalledWith('docker', ['build', '-t', 'runner', '-f', './docker/Dockerfile', '--build-arg', 'GITHUB_RUNNER_VERSION=1.111.1', '--build-arg', 'GITHUB_RUNNER_CHECKSUMS=123456789', '.'], { shell: true });
