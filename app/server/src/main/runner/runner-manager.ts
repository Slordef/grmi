...
export class RunnerManager implements RunManager {
    ...
    checkVersionRunner(): void {
        this.core.adapter<Fetcher>('Fetcher').fetch('https://api.github.com/repos/actions/runner/releases/latest').then((res: any) => {
    ...
    public setupDockerImage(version: string, checksums: string): void {
        const ps = spawn('docker', ['build', '-t', 'runner', '-f', './docker/Dockerfile', '--build-arg', `GITHUB_RUNNER_VERSION=${version}`, '--build-arg', `GITHUB_RUNNER_CHECKSUMS=${checksums}`, '.']);
