...
export class RunnerContainer {
    ...
    run(): void {
        ...
        const ps = spawn('docker', ['run', '-d', '-v', '/var/run/docker.sock:/var/run/docker.sock:rw', '-e', `GITHUB_REPOSITORY_URL=${this.url}`, '-e', `GITHUB_RUNNER_TOKEN=${this.token}`, '-e', `GITHUB_RUNNER_NAME=${this.name}`, '-e', `GITHUB_RUNNER_LABELS=${this.labels.join(',')}`, 'runner']);
        ps.stdout.on('data', (data) => {
            if (this.dockerID == '') {
                ...
                const lg = spawn('docker', ['logs', '-f', this.dockerID]);
    ...
    remove(): void {
        ...
        spawn('docker', ['rm', '-f', this.dockerID]);
